...
**2026-08-19 갱신:** 네이버 서치어드바이저 소유확인 ✅ · 사이트맵 제출 ✅ · 웹페이지 수집 요청 5건 ✅ · IndexNow ✅.
구글: `/`·`/tw.html` 은 이미 색인됨, en/ja/zh 는 「우선순위 크롤링 대기열」에 넣음. 며칠 뒤 GSC 「페이지」에서
5개 전부 색인됐는지, `/` 의 표준 URL 이 `/` 로 잡히는지(초기 크롤에 tw.html 로 보였음) 확인.

**구글 비즈니스 프로필 ✅ 2026-08-19** — 이 구글 계정이 청우해장(4.8★·46) 프로필 소유자였습니다.
웹사이트에 `https://cheongwoohaejang.com/` 등록(검토 대기 최대 10분). CID `14305556628635675431` 을
`store.mjs` 에 넣어 사이트의 구글 링크가 프로필로 직행. GSC 도 en/ja/zh 색인 요청 완료.
